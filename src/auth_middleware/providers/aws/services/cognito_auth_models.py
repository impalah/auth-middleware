...
class ChallengeRequest(BaseModel):
    """Payload for responding to an authentication challenge.

    Handles MFA or forced password change scenarios.
    """
    ...
    username: str = Field(
        ..., description="Username submitted at the initial login step"
    )
...
class ChallengeResponse(BaseModel):
    ...
    session: str = Field(
        ..., description="Opaque session token — send back in /auth/login/challenge"
    )


class LoginResponse(BaseModel):
    """Login result: tokens on success or a challenge if further action is required."""
