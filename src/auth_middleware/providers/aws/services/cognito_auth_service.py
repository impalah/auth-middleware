...
def _error_code(exc: Exception) -> str | None:
    """Extract Cognito error code from a ClientError.

    Does not import botocore at module level.
    """
...
class CognitoAuthService:
    ...
    def _auth_params(self, username: str, password: str) -> dict[str, str]:
        """Build the AUTH_PARAMETERS dict for Cognito InitiateAuth.

        Adds SECRET_HASH when required.
        """
    ...
    def _map_challenge(self, cognito_name: str) -> str:
        """Translate a Cognito challenge name to this application's simplified name."""
    ...
    def login(self, username: str, password: str) -> LoginResponse:
        """Initiate password-based auth via Cognito; returns tokens or a challenge."""
        try:
        ...
        return LoginResponse(
            tokens=self._tokens_from_result(resp["AuthenticationResult"])
        )
    ...
    def respond_to_challenge(
        ...
        cognito_challenge = {v: k for k, v in _CHALLENGE_MAP.items()}.get(
            challenge, challenge
        )
    ...
    def forgot_password(self, username: str) -> ForgotPasswordResponse:
        """Send a password-reset code to the user's registered email or phone."""
    ...
    def reset_password(
        ...
        """Complete the password-reset flow using the code sent to the user."""
        params: dict[str, Any] = {
            "ClientId": self._client_id,
            "Username": username,
    ...
    def setup_totp(self, access_token: str) -> TotpSetupResponse:
        """Begin TOTP MFA setup; returns the shared secret and QR URI for apps."""
        try:
