...
from auth_middleware.providers.aws.aws_credentials import AWSCredentials
from auth_middleware.providers.aws.cognito_provider import CognitoProvider
from auth_middleware.providers.aws.identity_pool_client import CognitoIdentityClient
from auth_middleware.providers.aws.identity_pool_settings import IdentityPoolSettings
...
class IdentityPoolProvider(CognitoProvider):
    ...
    def __init__(
        ...
        if settings:
            ...
            self.identity_pool_client = CognitoIdentityClient(
                region=settings.identity_pool_region or settings.user_pool_region,
                identity_pool_id=settings.identity_pool_id,
                user_pool_id=settings.user_pool_id or "",
            )
            ...
            pool_id = settings.identity_pool_id
            logger.debug(f"IdentityPoolProvider initialized with pool {pool_id}")
    ...
    async def get_aws_credentials(
        ...
        Args:
            token: JWT token from Cognito User Pool (string or
        ...
        token_str: str = str(token) if not isinstance(token, str) else token
    ...
    def clear_credentials_cache(self, user_id: str | None = None) -> None:
    ...
    async def __aenter__(self) -> IdentityPoolProvider:
