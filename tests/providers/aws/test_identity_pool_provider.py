...
from auth_middleware.providers.aws.identity_pool_provider import IdentityPoolProvider
...
class TestIdentityPoolProvider:
    ...
    def mock_jwt_token(self):
        ...
        token.__str__ = MagicMock(return_value="mock-jwt-token-string")  # type: ignore[method-assign]
        return token
    ...
    async def test_credentials_cache_expiration(self, settings, mock_jwt_token):
        ...
        # Create expiring credentials (4 min from now - within the 5-min buffer)
    ...
    async def test_cache_with_different_users(self, settings):
        ...
        token1.__str__ = MagicMock(return_value="token-1")  # type: ignore[method-assign]

        token2 = MagicMock(spec=JWTAuthorizationCredentials)
        token2.claims = {"sub": "user-2", "cognito:username": "user2"}
        token2.__str__ = MagicMock(return_value="token-2")  # type: ignore[method-assign]
