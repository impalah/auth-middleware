...
@patch(
    "auth_middleware.providers.aws.cognito_provider.httpx.AsyncClient.get",
    return_value=httpx.Response(200, json={"keys": [{"kid": "key1"}, {"kid": "key2"}]}, request=_MOCK_REQUEST),
)
@pytest.mark.asyncio
...
async def test_verify_token_no_hmac_key_candidate(mocker, cognito_provider):
    ...
    assert exc_info.value.detail == "No public key found!"
