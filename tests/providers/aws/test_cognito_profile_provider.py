...
from auth_middleware.providers.aws.cognito_profile_provider import (
...
class TestCognitoProfileProviderInit:
    ...
    def test_creates_boto3_client_when_not_provided(self):
        ...
        mock_boto3.assert_called_once_with("cognito-idp", region_name="eu-west-1")
        assert provider._client is mock_client
...
class TestFetchProfile:
    ...
    async def test_returns_empty_dict_on_exception_without_response_attr(self):
        """Exception without .response attribute returns empty dict.

        Excludes the UserNotFoundException branch.
        """
        mock_client = MagicMock()
