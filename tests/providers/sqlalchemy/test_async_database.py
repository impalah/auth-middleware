...
class TestAsyncDatabase:
    ...
    @patch("auth_middleware.providers.sqlalchemy.async_database.create_async_engine")
    @patch("auth_middleware.providers.sqlalchemy.async_database.async_sessionmaker")
    def test_initialize_class_method(self, mock_async_sessionmaker, mock_create_engine):
        ...
        mock_async_sessionmaker.assert_called_once_with(
            autocommit=False, autoflush=False, bind=mock_engine, class_=AsyncSession
        )
    ...
    @patch("auth_middleware.providers.sqlalchemy.async_database.settings")
    @patch("auth_middleware.providers.sqlalchemy.async_database.create_async_engine")
    @patch("auth_middleware.providers.sqlalchemy.async_database.async_sessionmaker")
    def test_init_engine(
        self, mock_async_sessionmaker, mock_create_engine, mock_settings
    ):
    ...
    def test_init_engine_only_once(self):
        ...
        with patch.object(db, "_engine", None):
            with patch(
                "auth_middleware.providers.sqlalchemy.async_database.create_async_engine"
            ) as mock_create:
                mock_engine = Mock()
    ...
    def test_new_method_logging(self):
        ...
        with patch(
            "auth_middleware.providers.sqlalchemy.async_database.logger"
        ) as mock_logger:
            # First instance
            AsyncDatabase()
