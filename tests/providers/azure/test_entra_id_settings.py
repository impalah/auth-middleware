...
from auth_middleware.providers.azure.settings import ModuleSettings, settings
...
class TestModuleSettings:
    ...
    def test_module_settings_from_environment(self):
        ...
        with patch.dict(os.environ, env_vars, clear=False):
            ...
            import auth_middleware.providers.azure.settings as entra_id_settings_module

            test_settings = entra_id_settings_module.ModuleSettings()

            assert (
                test_settings.AUTH_PROVIDER_AZURE_ENTRA_ID_TENANT_ID == "test-tenant-id"
            )
            assert (
                test_settings.AUTH_PROVIDER_AZURE_ENTRA_ID_AUDIENCE_ID
                == "test-audience-id"
            )
            assert (
                test_settings.AUTH_PROVIDER_AZURE_ENTRA_ID_JWKS_URL_TEMPLATE
                == "https://custom.url/{}/jwks.json"
            )
