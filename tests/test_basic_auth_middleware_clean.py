...
class TestBasicAuthMiddleware:
    ...
    @pytest.fixture
    def basic_auth_middleware(self, mock_credentials_repository):
        """Create a BasicAuthMiddleware instance with mocked repository."""
        from unittest.mock import Mock

        mock_app = Mock()
        return BasicAuthMiddleware(
            app=mock_app, credentials_repository=mock_credentials_repository
        )
    ...
    def user_credentials(self):
        ...
        hashed_password = hashlib.sha256(b"testpass").hexdigest()
        return UserCredentials(
            id="testuser",  # Use id instead of username
            name="testuser",
            hashed_password=hashed_password,  # hashed_password field name
            groups=["admin", "user"],
        )

    def test_init_with_credentials_repository(self, mock_credentials_repository):
    ...
    async def test_dispatch_success(
        self, basic_auth_middleware, mock_request, valid_credentials, user_credentials
    ):
        ...
        assert mock_request.state.current_user.id == username

    @pytest.mark.asyncio
    async def test_dispatch_invalid_authorization_exception(
        ...
        with patch(
            ...
            with patch.object(
                ...
                "get_current_user",
                side_effect=InvalidAuthorizationException(
                    status_code=403, detail="Test error"
                ),
            ):
                response = await basic_auth_middleware.dispatch(mock_request, call_next)
    ...
    @pytest.mark.asyncio
    async def test_get_credentials_middleware_disabled(
        self, basic_auth_middleware, mock_request
    ):
        ...
        with patch(
            ...
            credentials = basic_auth_middleware.get_credentials(mock_request)
            assert credentials is None

    @pytest.mark.asyncio
    async def test_get_credentials_valid_basic_auth(
    ...
    async def test_get_credentials_no_authorization_header(
        ...
        with patch(
            ...
            with pytest.raises(InvalidAuthorizationException):
                basic_auth_middleware.get_credentials(mock_request)
    ...
    @pytest.mark.asyncio
    async def test_get_credentials_invalid_base64(
        self, basic_auth_middleware, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_credentials_missing_password_separator(
        self, basic_auth_middleware, mock_request
    ):
        """Test get_credentials with missing colon separator."""
        credentials_without_colon = base64.b64encode(b"usernameonly").decode("ascii")
        mock_request.headers = Headers(
            {"authorization": f"Basic {credentials_without_colon}"}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_credentials_empty_credentials(
        self, basic_auth_middleware, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_invalid_password(
        self, basic_auth_middleware, mock_request, user_credentials
    ):
    ...
    async def test_get_current_user_with_groups(
        ...
        user_credentials = UserCredentials(
            id=username,  # Use id instead of username
            name=username,
    ...
    @pytest.mark.asyncio
    async def test_edge_case_special_characters_in_credentials(
        self, basic_auth_middleware, mock_request
    ):
        ...
        with patch(
            ...
            creds = basic_auth_middleware.get_credentials(mock_request)

        assert creds[0] == username
        assert creds[1] == password

    @pytest.mark.asyncio
    async def test_edge_case_unicode_credentials(
        self, basic_auth_middleware, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_multiple_colons_in_password(
        self, basic_auth_middleware, mock_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_case_insensitive_basic_scheme(
        self, basic_auth_middleware, mock_request, valid_credentials
    ):
