...
class TestJsonCredentialsRepository:
    ...
    def test_init_loads_json_file(
        self, mock_settings, temp_json_file, sample_json_data
    ):
        """Test that __init__ loads the JSON file correctly."""
        mock_settings.AUTH_MIDDLEWARE_JSON_REPOSITORY_PATH = os.path.basename(
            temp_json_file
        )
    ...
    @pytest.mark.asyncio
    @patch("auth_middleware.repository.json_credentials_repository.settings")
    async def test_get_by_id_user_without_optional_fields(
        self, mock_settings, temp_json_file
    ):
    ...
    def test_repository_inheritance(self):
        ...
        from auth_middleware.contracts.credentials_repository import (
        ...
        with patch(
            "builtins.open",
            mock_open(read_data='{"test": {"name": "Test", "hashed_pwd": "pwd"}}'),
        ):
            with patch(
                "auth_middleware.repository.json_credentials_repository.settings"
            ) as mock_settings:
