...
class TestRequirePermissions:
    ...
    async def test_require_permissions_success(self, mock_request_with_user):
        ...
        with patch(
            "auth_middleware.guards.functions.PermissionsChecker"
        ) as mock_checker:
...
class TestRequireGroups:
    ...
    async def test_require_groups_success(self, mock_request_with_user):
        ...
        # Mock GroupChecker
        with patch("auth_middleware.guards.functions.GroupChecker") as mock_checker:
            mock_checker_instance = AsyncMock()
            mock_checker.return_value = mock_checker_instance
...
class TestFunctionsIntegration:
    ...
    async def test_multiple_requirements_together(self, mock_request_with_user):
        ...
        # Test require_groups
        with patch(
            "auth_middleware.guards.functions.GroupChecker"
        ) as mock_group_checker:
...
class TestRequireRoles:
    ...
    async def test_require_roles_passes_when_role_matches(self, mock_request_with_user):
        with patch(
            "auth_middleware.guards.role_checker.settings.AUTH_MIDDLEWARE_DISABLED",
            False,
        ):
            mock_user = Mock()
    ...
    @pytest.mark.asyncio
    async def test_require_roles_raises_403_when_role_missing(
        self, mock_request_with_user
    ):
        with patch(
            "auth_middleware.guards.role_checker.settings.AUTH_MIDDLEWARE_DISABLED",
            False,
...
class TestHasRoles:
    ...
    @pytest.mark.asyncio
    async def test_has_roles_returns_false_when_no_user(
        self, mock_request_without_user
    ):
        with patch(
            "auth_middleware.guards.role_checker.settings.AUTH_MIDDLEWARE_DISABLED",
            False,
