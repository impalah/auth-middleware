...
from auth_middleware.services.cognito_auth_models import LoginResponse, TokenResponse
from auth_middleware.services.cognito_auth_service import CognitoAuthService
...
def test_login_raises_authentication_error_on_password_reset_required(svc):
    svc._cognito.initiate_auth.side_effect = _client_error("PasswordResetRequiredException")
...
def test_respond_to_challenge_raises_invalid_on_code_mismatch(svc):
    svc._cognito.respond_to_auth_challenge.side_effect = _client_error("CodeMismatchException")
...
def test_respond_to_challenge_raises_password_policy_error(svc):
    svc._cognito.respond_to_auth_challenge.side_effect = _client_error("InvalidPasswordException")
...
def test_get_user_returns_empty_groups_on_group_error(svc):
    ...
    svc._cognito.admin_list_groups_for_user.side_effect = _client_error("AccessDeniedException")
...
def test_reset_password_raises_invalid_on_code_mismatch(svc):
    svc._cognito.confirm_forgot_password.side_effect = _client_error("CodeMismatchException")
...
def test_reset_password_raises_invalid_on_expired_code(svc):
    svc._cognito.confirm_forgot_password.side_effect = _client_error("ExpiredCodeException")
...
def test_reset_password_raises_policy_error(svc):
    svc._cognito.confirm_forgot_password.side_effect = _client_error("InvalidPasswordException")
...
def test_reset_password_raises_user_not_found(svc):
    svc._cognito.confirm_forgot_password.side_effect = _client_error("UserNotFoundException")
...
def test_setup_totp_returns_secret_and_qr(svc):
    svc._cognito.associate_software_token.return_value = {"SecretCode": "JBSWY3DPEHPK3PXP"}
...
def test_setup_totp_raises_mfa_setup_error_on_failure(svc):
    svc._cognito.associate_software_token.side_effect = _client_error("InvalidParameterException")
...
def test_verify_totp_raises_mfa_setup_error_on_code_mismatch(svc):
    svc._cognito.verify_software_token.side_effect = _client_error("CodeMismatchException")
...
def test_disable_totp_raises_mfa_setup_error_on_failure(svc):
    svc._cognito.set_user_mfa_preference.side_effect = _client_error("NotAuthorizedException")
